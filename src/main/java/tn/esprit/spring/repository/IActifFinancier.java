...
List<ActifFinancier> findByNomFond(Fond nom_fond) ;
	...
	@Query(value = "SELECT * FROM actif_financier WHERE nom_fond='Euro_Croissance' and  etat='En cours' and user_actif_id_user=:w ",nativeQuery = true)
	 List<ActifFinancier> listemontant_actuelEuroCroissanceparUser( @Param("w") Long w );

	@Query(value = "SELECT * FROM actif_financier WHERE nom_fond='Fond_Euro' and  etat='En cours' and user_actif_id_user=:w ",nativeQuery = true)
